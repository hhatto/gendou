...
let close: sinon.SinonStub<
	...
	Promise<boolean>
>
let caluculateContriburionsCount: sinon.SinonStub<
	[githubId: string],
...
test.before(() => {
	...
	caluculateContriburionsCount = sinon.stub(
		contributions_modules,
...
test('reward is not found.', async (t) => {
	...
	caluculateContriburionsCount
		.withArgs('github-3')
		.resolves([bignumber(0), bignumber(1)])
	calculateGeometricMean
		.withArgs([bignumber(0), bignumber(1)])
		.returns(bignumber('1.5'))
	getRewordRecordByCommitCount
		.withArgs({ db: true } as any, 1)
		.resolves(undefined)
...
test('get reward.', async (t) => {
	...
	caluculateContriburionsCount
		.withArgs('github-4')
		.resolves([bignumber(1), bignumber(2)])
	calculateGeometricMean
		.withArgs([bignumber(1), bignumber(2)])
		.returns(bignumber('2.5'))
	getRewordRecordByCommitCount
		.withArgs({ db: true } as any, 2)
...
test('get reward record.', async (t) => {
	...
	caluculateContriburionsCount
		.withArgs('github-5')
		.resolves([bignumber(2), bignumber(3)])
	calculateGeometricMean
		.withArgs([bignumber(2), bignumber(3)])
		.returns(bignumber('3.5'))
	getRewordRecordByCommitCount
		.withArgs({ db: true } as any, 3)
		.resolves(dummyRewward)
	const [res1, res2] = await getRewardFromGithubId(
		{ db: true } as any,
		'github-5'
	)
	t.is(res1!.id, 5)
	t.is(res1!.commit_lower_limit, 10)
	t.is(res1!.commit_upper_limit, 200)
	t.is(res1!.reward, '10000000000000000000')
	t.is(res1!.rank, 5)
	t.is(res2, 3)
})
